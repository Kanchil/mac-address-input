<template>
  <div class="mac-address__container">
    <ul class="mac-address__list">
      <li v-for="index in 5" :key="index" :class="['mac-address__inner', { 'no-valid': noValid }]">
...
export default {
  ...
  methods: {
    ...
    inputAction (event, index) {
      ...
      this.noValid = !this.validateMac(this.macList)
      this.$emit('input', this.arrayToMac(this.macList), this.noValid)
    },
    macToArray (mac) {
