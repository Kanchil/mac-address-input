  ...
  methods: {
    keydownAction (event, index) {
      if (!this.macList[index - 1]) {
        ...
        this.$set(this.macList, index - 1, 'SSS')
      }
    },
    pasteAction (event) {
    ...
    inputAction (event, index) {
      ...
      // event.inputType !== 'deleteContentBackward' && this.$set(this.macList, index - 1, (this.macList[index - 1] || '').replace(/[^0-9a-zA-Z]/g, '').toUpperCase().slice(0, 2))
